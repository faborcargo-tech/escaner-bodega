...
def *get_public_or_signed_url(path: str) -> str | None:
try:
url = supabase.storage.from*(STORAGE_BUCKET).get_public_url(path)
...
if not rows:
[st.info](http://st.info/)("No hay registros aún.")
return
cols = (
...
mime="application/pdf",
key=f"dl_{asign}*{guia}*{time.time()}",
)
else:
c[4].write("No válido")
except Exception:
...
if order_id:
r = meli_request("GET", f"[https://api.mercadolibre.com/orders/{order_id}](https://api.mercadolibre.com/orders/%7Border_id%7D)")
if r.status_code == 200:
...
if pack_id:
r = meli_request("GET", f"[https://api.mercadolibre.com/packs/{pack_id}](https://api.mercadolibre.com/packs/%7Bpack_id%7D)")
if r.status_code == 200:
...
url = f"[https://api.mercadolibre.com/shipments/{shipment_id}/process/ready_to_ship](https://api.mercadolibre.com/shipments/%7Bshipment_id%7D/process/ready_to_ship)"
